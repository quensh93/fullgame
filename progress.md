...
- New prompt: Tarneeb round-result dialog should dismiss automatically with the next round start, without a Continue button or dead gap between rounds.
- Shortened `TarneebEngineService` round transition delay to 2s, removed the manual continue button from both desktop/mobile Tarneeb result overlays, and added an auto-dismiss path in `useTarneebGame` that also flushes queued `GAME_FINISHED` results when the overlay clears.
- `./gradlew compileJava --no-daemon` passed in `gameBackend` and `npm run build` passed in `gameweb`.
- Playwright smoke testing is still blocked in this environment because the local `playwright` package is not installed for `/Users/sajadrahmanipour/.codex/skills/develop-web-game/scripts/web_game_playwright_client.js`.
...
- Verification passed for `npm run build` in `gameweb` and `git diff --check` on `gameweb/src/app/pages/ConnectFourGameBase.tsx`.
- New prompt: in Tarneeb desktop, make the right sidebar match the Hokm desktop sidebar exactly.
- Reworked the Tarneeb desktop right rail to mirror the Hokm sidebar structure: same narrow rail width, header row, `first to` strip, two compact team score cards, Hokm-style round history list, and the shared unwrapped `QuickChatPanel` docked at the bottom.
- Verification passed for `npm run build` in `gameweb` and `git diff --check` on `gameweb/src/app/pages/DesktopTarneebScreen.tsx`.
- Follow-up prompt: the Tarneeb mobile turn-status row was still changing the page height when the timer swapped to `Waiting for turn...`.
- Moved the mobile Tarneeb turn indicator into a single reserved block below the hand so the timer bar and waiting text occupy the same space, matching the Hokm layout and preventing table/player vertical shift.
- Verification passed for `npm run build` in `gameweb` and `git diff --check` on `gameweb/src/app/pages/TarneebScreen.tsx`.
- Follow-up prompt: the inter-round result dialog was showing the raw `common.continue` key instead of localized text.
- Added `common.continue` to the shared translation tables for English, Persian, Arabic, and Turkish so round-result continue buttons now resolve correctly instead of leaking the key.
- Verification passed for `npm run build` in `gameweb` and `git diff --check` on `gameweb/src/app/i18n/translations.ts`.
- Follow-up prompt: the Tarneeb mobile page still shifted when the active turn changed.
- Moved the Tarneeb mobile hand/status area into a fixed-height `bottomReserve` slot on `MobileTrickTakingArena`, matching the Hokm pattern so the table and player positions no longer jump when the turn bar swaps between timer and waiting text.
- Verification passed for `npm run build` in `gameweb` and `git diff --check` on `gameweb/src/app/pages/TarneebScreen.tsx`.
- Follow-up prompt: the turn-status label needed to say just `Waiting`, and the round-result dialog needed to auto-dismiss and appear at the table center instead of the page center.
- Added a shared `common.waiting` translation, swapped the Tarneeb waiting label to that short form, and moved the round-result overlay into the mobile table layer so it is centered on the board and auto-hides after about 1.8 seconds unless dismissed manually.
- Verification passed for `npm run build` in `gameweb` and `git diff --check` on `gameweb/src/app/pages/TarneebScreen.tsx`, `gameweb/src/app/hooks/useTarneebGame.ts`, and `gameweb/src/app/i18n/translations.ts`.
...
- Verification passed for `npm run build` in `gameweb` and `git diff --check` on `gameweb/src/app/i18n/translations.ts`.
- New prompt: in Hokm spectator mode, the bottom player's hidden hand showed `Waiting` under the cards and never showed that player's turn progress bar.
- Updated Hokm desktop/mobile spectator bottom-seat UI so it now uses the actual bottom player's name instead of the generic waiting label, and the turn progress strip now keys off `seatMap.bottom.isCurrentTurn` rather than `isMyTurn`.
- Also aligned the bottom quick-chat anchor in spectator mode to the bottom viewed player instead of the spectator session user.
- Verification passed for `npm run build` in `gameweb`.
- New prompt: in Hokm spectator mode, the sidebar/menu should also show round history the same way players do.
- Refactored Hokm desktop/mobile round-history accumulation to use the spectator's viewed-team perspective instead of the session user's team, so W/L and per-round score lines now map correctly for spectator mode.
- Added snapshot-backed round-history sync in both Hokm screens: if spectator mode misses a `ROUND_ENDED` action but later receives updated round-win totals in authoritative state, the sidebar/menu still appends that round locally without duplicating entries already added by the explicit event.
- This still does not backfill full pre-join round history from the server, because Hokm round history is not yet persisted in backend payloads; the new logic covers rounds observed after the initial spectator hydration and closes the missing-history gap caused by event-only local handling.
- Verification passed for `npm run build` and `git diff --check` in `gameweb`.
- Follow-up: the snapshot-backed Hokm spectator round-history attempt caused duplicate `0-0` entries for players and still did not make spectator history appear reliably.
- Removed that snapshot-derived append path from Hokm desktop/mobile so round history is once again appended only from the explicit `ROUND_ENDED` event, eliminating the duplicate bogus entry.
- Fixed the actual root cause in backend `WebSocketRoomService`: Hokm spectator sessions now receive delayed `ROUND_ENDED` game actions too, just like the delayed snapshot flow, so spectator round history can populate from the correct round-end payload instead of guessing from reset state.
- Kept the spectator/team-perspective round-history mapping on the frontend, so the delayed Hokm `ROUND_ENDED` event now renders W/L and round scores from the watched friend's team perspective.
- Verification passed for `npm run build` in `gameweb`, `./gradlew compileJava --no-daemon` in `gameBackend`, and `git diff --check`.
- New prompt: in Hokm, when a round ends, show a cinematic round-result view for about 2 seconds with win/loss plus that round's score, and sync it cleanly with the existing `Get Ready` flow before the next round starts.
- Added a shared `HokmRoundTransitionOverlay` for both mobile and desktop web Hokm so the round-result card and the `Get Ready` countdown now use the same motion language, blur/scrim treatment, and score presentation instead of two unrelated overlays.
- Reworked Hokm round-transition timing on both `GameScreen.tsx` and `DesktopGameScreen.tsx`: `ROUND_ENDED` now locks the transition locally, keeps the result overlay visible for a minimum ~2 seconds, and only then allows the next-round countdown to begin, even if `NEW_ROUND_STARTED` arrives earlier from the server.
- The new round-result overlay now shows the actual round score (`tricks this round`) plus the overall match standing, instead of the old misleading `roundScore` line that was only showing total round wins.
- Added `hokm.round` translations in EN/FA/AR/TR so the transition overlay can label the active round cleanly in all supported locales.
- Verification passed for `pnpm build` inside `gameweb` and `git diff --check` for `src/app/components/HokmRoundTransitionOverlay.tsx`, `src/app/pages/GameScreen.tsx`, `src/app/pages/DesktopGameScreen.tsx`, `src/app/i18n/translations.ts`, and `progress.md`.
- Browser Playwright verification was not run in this turn; earlier workspace notes still indicate the local web-game Playwright client is blocked here because the `playwright` package is missing.
- Follow-up: restored the original Hokm `Get Ready` / countdown design and animation on both mobile and desktop after the transition refactor changed it too much; the new round-result overlay remains, but countdown visuals are back to the previous implementation while the improved timing sync is preserved.
- Follow-up prompt: make the Hokm round-result overlay much stronger and more cinematic without touching `Get Ready`.
- Rebuilt `HokmRoundTransitionOverlay` into a more dramatic round-result scene: royal gold/emerald win palette, crimson/ice lose palette, rotating light halo, radiant beams, floating suit sigils, stronger emblem reveal, and split score slabs for both round score and overall match score.
- Adjusted Hokm round-result audio sequencing on both mobile and desktop so the overlay now enters with `whoosh`, then `reveal`, then the final `win` / `lose` sting, instead of jumping straight to the end-state sound.
- Verification passed for `pnpm build` in `gameweb` and `git diff --check` on `src/app/components/HokmRoundTransitionOverlay.tsx`, `src/app/pages/GameScreen.tsx`, and `src/app/pages/DesktopGameScreen.tsx`.
- Continued the in-flight spectator feature work by adding focused backend coverage for `SpectatorBetService`: locked the `create bet`, `round settlement payout split`, and `cancel on spectator disconnect` contracts so the new betting flow is verified beyond compile/build-only signals.
- Also aligned the affected `WebSocketRoomService` tests with the current constructor and cached room-list DTO path, so the supporting websocket room-list/user-status coverage compiles and runs against the latest service wiring.
- Verification passed for `./gradlew test --tests com.gameapp.game.services.SpectatorBetServiceTest --tests com.gameapp.game.services.UserGameSummaryServiceTest --tests com.gameapp.game.ImprovedWebSocketConfigSpectatorSubscriptionTest --tests com.gameapp.game.services.WebSocketRoomServiceUserStatusTest --tests com.gameapp.game.services.WebSocketRoomServiceRoomListOnlineCountTest --no-daemon`, and `./gradlew compileJava --no-daemon` had already passed earlier in this turn.
- Follow-up prompt: spectator betting should also be allowed for private RPS rooms, since any spectator who joins already knows the password and has been authorized into the room.
- Relaxed the spectator-bet room-type eligibility in `SpectatorBetService` so casual `PRIVATE` RPS rooms now qualify alongside `PUBLIC` rooms; the other guards remain unchanged (`IN_PROGRESS`, no bot assist, zero spectator delay, exactly two human players, and an active spectator session).
- Added focused coverage to lock that private-room path, and verification passed for `./gradlew test --tests com.gameapp.game.services.SpectatorBetServiceTest --no-daemon`.
- Follow-up prompt: bring the same spectator-betting flow to Backgammon next.
- Generalized `SpectatorBetService` eligibility beyond RPS so it now understands Backgammon too: bets are still limited to casual two-human rooms with zero spectator delay, but Backgammon opens the market only in the between-hands `HAND_FINISHED` window and stores the room's real `gameType` on each bet.
- Wired `BackgammonEngineService` into the betting lifecycle: when a hand finishes it now settles `NEXT_ROUND_WINNER`, opens the next-hand market during the between-hands pause, closes pending offers again when the next hand actually starts, and settles `MATCH_WINNER` on natural finish/forfeit.
- Hooked the Backgammon web screen into the spectator-bet UI by adding `useSpectatorBets`, passing live bet state into `InGameSpectatorPanel`, and exposing the same composer CTA/overlay that RPS spectators already use.
- Added focused tests for Backgammon spectator-bet creation/window rules plus engine wiring, and verification passed for `./gradlew test --tests com.gameapp.game.services.SpectatorBetServiceTest --tests com.gameapp.game.services.BackgammonEngineServiceBotContractTest --tests com.gameapp.game.services.BackgammonEngineServiceRulesTest --no-daemon`, `npm run build` in `gameweb`, and `git diff --check` on the edited files.
- Follow-up prompt: the new spectator-bet UI was showing raw `spectator.bet.*` keys instead of localized copy.
- Added the full `spectator.bet.*` translation set in EN/FA/AR/TR, covering the composer, panel sections, action labels, statuses, prediction sentence, and leaderboard labels so the spectator-bet flow no longer falls back to raw keys in any supported app language.
- Verification passed for `npm run build` in `gameweb`, `git diff --check` on `src/app/i18n/translations.ts`, and the expected spectator-bet key set is now present in the translation table for all four locales.
- Follow-up prompt: in Backgammon, spectator bets should not wait for the between-hands pause; spectators should be able to bet during an active hand on the upcoming hand winner and on the overall match winner.
- Reworked Backgammon spectator-bet market semantics in `SpectatorBetService`: while the match is in progress, both `NEXT_ROUND_WINNER` and `MATCH_WINNER` are now open during the active hand too; `NEXT_ROUND_WINNER` dynamically targets the upcoming hand number (`currentRound + 1` during a live hand, current round during the between-hands pause), while `MATCH_WINNER` remains valid across hand transitions instead of being expired on every hand start.
- Also aligned snapshot visibility so resolved Backgammon `NEXT_ROUND_WINNER` rows remain visible through the covered hand and disappear once the match advances beyond that hand, instead of vanishing immediately just because the market is open again.
- Verification passed for `./gradlew test --tests com.gameapp.game.services.SpectatorBetServiceTest --tests com.gameapp.game.services.BackgammonEngineServiceBotContractTest --tests com.gameapp.game.services.BackgammonEngineServiceRulesTest --no-daemon`.
- Follow-up prompt: move spectator-bet offer expiry into the admin game settings dialog and raise the default from the hardcoded short values to 30 seconds.
- Added a persisted `spectator_bet_offer_expiry_seconds` field to `game_configs`, exposed it through the admin game-config DTO/request flow, and surfaced it in the admin game settings dialog so each game can now configure spectator-bet offer expiry alongside spectator delay and timer settings.
- `SpectatorBetService` now resolves bet-offer TTL from `GameConfigService` instead of hardcoded per-game values, and the new resolver defaults to 30 seconds for all games when a config is missing or unset; the migration also backfills existing rows to 30.
- Verification passed for `./gradlew test --tests com.gameapp.game.services.GameConfigServiceTimerSettingsTest --tests com.gameapp.game.services.SpectatorBetServiceTest --tests com.gameapp.game.controllers.AdminGameConfigControllerTest --tests com.gameapp.game.controllers.PublicGameConfigControllerTest --no-daemon` and `npm run build` in `gameweb`.
- New prompt: in Hokm mobile web, the played top-seat and bottom-seat table cards overlap, so the 4-card center layout should be adjusted to keep every played card readable.
- Increased the Hokm mobile center trick area in `gameweb/src/app/pages/GameScreen.tsx` and made it a `relative` positioning container, so the shared top/bottom/left/right trick-card slots render inside a stable minimum box instead of collapsing too tightly on narrow mobile widths.
- Follow-up prompt: make those Hokm mobile center cards 20% larger because they still look too small.
- Increased the Hokm mobile trick-card render scale to `1.2x` and widened the center trick box a bit more in `gameweb/src/app/pages/GameScreen.tsx` so the larger played cards stay readable without reintroducing overlap.
- Follow-up prompt: in Hokm mobile, player stickers are rendering underneath the center table cards and should appear above them.
- Raised the mobile top/left/right seat layers above the center-card layer in `gameweb/src/app/components/mobileTable/MobileTrickTakingArena.tsx`, so `QuickChatBubble` stickers can visually cross into the middle of the table instead of getting clipped beneath trick cards.
- New prompt: in mobile Shelem, the local player never detects their turn and stays stuck on `waiting for turn`.
- Root cause was the Shelem snapshot normalizer re-reading merged live state after every `GAME_ACTION` but only trusting legacy snapshot fields (`currentPlayerId`, `gameRoom.players.user.id`), so mobile turn ownership could be overwritten back to a stale/empty state even when live `currentTurnPlayerId` and top-level `players[].playerId` were correct.
- Updated `gameweb/src/app/services/gameStateMappers.ts` so `normalizeShelemSnapshot` now follows the same resilient pattern as Tarneeb: it accepts merged top-level `players`, reads `playerId/userId/ user.id`, and falls back to `currentTurnPlayerId`/top-level bid fields when `currentPlayerId` is absent.
- Verification passed for `pnpm exec esbuild src/app/pages/ShelemScreen.tsx --bundle --platform=browser --format=esm --outfile=/tmp/shelem-screen-check.js` and `git diff --check -- src/app/services/gameStateMappers.ts`.
